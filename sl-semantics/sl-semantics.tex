...
Whenever we use the term \enquote*{model}, we will almost always mean a partial model.
We aren't generally concerned about the truth values of all of the sentences in \emph{SL}, but only those that are contained in the sentence or argument that we are analyzing.

A truth-table for a sentence shows its truth-value for every model.
